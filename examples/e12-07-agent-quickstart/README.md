...
2. **JVM 参数不可省略**:`--add-exports=java.base/jdk.internal.vm=ALL-UNNAMED` 是 JDK 21 下 Continuation 机制的硬性要求(官方 Quickstart 明示);集群模式追加到 `config.yaml` 的 `env.java.opts.all`。
