...
> 本仓库体量(教材 + 100+ Demo + 3 个生产级项目)必须分阶段构建。本文件是阶段划分、验收口径与 **Claude Code + open-gsd 接力协议** 的 SSOT。文档以实质内容与可读性为准，**不以行数作为完成态硬指标**。
...
| **P6 总装 QA** | 全仓交叉引用校验、违禁词扫描(内容禁令词表)、案例数盘点、README 终稿 | 第 10 部分 + 最终交付要求 | ✅ **可验证完成态**（QA-01/02 + ENG-01…04）：`bash scripts/qa_check.sh` 全绿；mains≥100；文档质量以实质内容为准（禁止注水刷行数）；`bash scripts/eng_audit.sh` ENG-01…04；扫描匹配整词内容禁令词表、不裸匹配「略」（D-08）；git tag 留给 `/gsd-complete-milestone`（D-12） |
