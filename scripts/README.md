...
| `qa_check.sh` | 五硬门：compose / 违禁词(含「省略」) / 断链 / mains≥100 / mvn compile 硬失败；文档行数仅诊断；末尾调用 eng_audit | `bash scripts/qa_check.sh` |
| `eng_audit.sh` | ENG-01…04 终检（版本抽样 / docs 编号 / baseline 证据 / CHANGELOG Unreleased；PHASES 严格断言见 07-03） | `bash scripts/eng_audit.sh` |
| `count_docs.py` | 文档行数诊断（excl `.planning`/`.git`；无最小值硬失败） | `python3 scripts/count_docs.py` |
